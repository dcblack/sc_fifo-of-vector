...
public:
  ...
  {
    auto elapsed_time = elapsed(); // grab time immediately (avoid overhead of remaining computations)
    auto result = "Timer"s;
    if( not m_timerName.empty() ) result += "("s + m_timerName + ")"s;
    result += ": "s;
    if( not reportName.empty() ) result += reportName + " "s;
    result += Chronout::to_string(elapsed_time, 1ms);
    return result;
  }

  void report(const std::string& reportName="") {
    #ifdef SYSTEMC_INCLUDED
    SC_REPORT_INFO_VERB( msg_type, to_string( reportName ).c_str(), sc_core::SC_NONE );
    #else
...
private:
  ...
  static void disclaimer() {
    ...
    #ifdef SYSTEMC_INCLUDED
      SC_REPORT_INFO_VERB( msg_type, text, ::sc_core::SC_NONE );
    #else
      std::cout << text << '\n';
    #endif
    once = true;
